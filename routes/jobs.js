...
const APPLICATIONS_TABLE = process.env.SUPABASE_JOB_APPLICATIONS_TABLE || 'job_applications';
...
function inferProvince(locationText = '', fallback = '') {
  ...
  if (/limpopo|tzaneen|polokwane|phalaborwa|hoedspruit|giyani|mokopane|thohoyandou|musina|bela-bela|lephalale|thabazimbi|burgersfort|modimolle|makhado|louis trichardt|marble hall|groblersdal/.test(combined)) {
    return 'Limpopo';
  }

  if (/gauteng|johannesburg|pretoria|tshwane|soweto|sandton|midrand|centurion|ekurhuleni/.test(combined)) {
    return 'Gauteng';
  }

  if (/mpumalanga|mbombela|nelspruit|witbank|emalahleni|secunda|white river/.test(combined)) {
    return 'Mpumalanga';
  }
  ...
  if (/free state|bloemfontein/.test(combined)) {
    return 'Free State';
  }

  if (/north west|rustenburg|mahikeng/.test(combined)) {
    return 'North West';
  }

  if (/northern cape|kimberley|kathu/.test(combined)) {
...
function isSupportedAdzunaLocation(location = '') {
  ...
  if (AFRICA_LOCATIONS.some((item) => normalizeLower(item) === loc) && loc !== 'south africa') {
    return false;
  }
...
function isLikelySouthAfricaJob(job = {}) {
  ...
  if (SOUTH_AFRICA_LOCATIONS.some((location) => combined.includes(normalizeLower(location)))) {
    return true;
  }
...
function detectIntent(rawQuery = '') {
  ...
  if (/\b(admin|administrator|administration|clerk|receptionist|data capturer|office assistant|personal assistant|pa|secretary|office)\b/.test(q)) {
    return 'admin';
  }

  if (/\b(cleaner|cleaning|housekeeper|housekeeping)\b/.test(q)) return 'cleaner';
  if (/\b(cashier|retail|shop assistant|sales assistant|merchandiser)\b/.test(q)) return 'retail';
  if (/\b(security|guard|armed response)\b/.test(q)) return 'security';
  if (/\b(general worker|packer|warehouse|picker|loader)\b/.test(q)) return 'general';
...
function buildKeywordPath(rawQuery = '') {
  ...
  if (intent === 'cleaner') return uniq(['cleaner', 'cleaning', 'housekeeper', 'housekeeping']);

  if (intent === 'retail') {
    return uniq(['cashier', 'retail assistant', 'shop assistant', 'sales assistant', 'merchandiser']);
  }

  if (intent === 'security') return uniq(['security guard', 'security officer', 'armed response', 'security']);

  if (intent === 'general') {
    return uniq(['general worker', 'packer', 'warehouse assistant', 'loader', 'picker']);
  }
...
function buildSearchPath(area = '') {
  ...
  if (GREATER_TZANEEN_VILLAGES_TOWNSHIPS.some((place) => normalizeLower(place) === lowerArea)) {
    path.push('Tzaneen');
  }

  if (
    GREATER_TZANEEN_VILLAGES_TOWNSHIPS.some((place) => normalizeLower(place) === lowerArea) ||
    LIMPOPO_TOWNS_CITIES.some((place) => normalizeLower(place) === lowerArea) ||
...
function localScore(job = {}, requestedArea = '') {
  ...
  const combined = normalizeLower(`${job.area} ${job.town} ${job.province} ${job.description}`);

  if (!area) return 0;
  if (combined.includes(area)) return 60;
  if (area.includes('tzaneen') && combined.includes('tzaneen')) return 60;
  if (area.includes('limpopo') && combined.includes('limpopo')) return 45;
...
function relevanceScore(job = {}, rawQuery = '', requestedArea = '') {
  ...
  if (intent === 'driver') {
    if (/\b(driver|drivers|salesman driver|bus driver|truck driver|ultra-heavy|heavy motor vehicle|code 10|code 14|courier|delivery driver|prdp|pdp)\b/.test(title)) {
      score += 120;
    }

    if (/\b(fleet|transport|logistics|warehouse|vehicle|depot|road supervisor|truck workshop)\b/.test(title)) {
      score += 70;
    }

    if (/\b(logistics|warehouse|transport|travel)\b/.test(category)) score += 35;

    if (/\bdriver|fleet|transport|logistics|truck|vehicle|courier|delivery|road|depot|bus|prdp|pdp|code 10|code 14\b/.test(combinedTitle)) {
      score += 35;
    }

    if (/\b(valid driver'?s? licence|valid drivers licence|driver's license|drivers license|own vehicle|driver licence|driver license)\b/.test(description) && score < 80) {
      score -= 90;
    }
  ...
  if (intent === 'admin') {
    if (/\b(admin|administrator|administration|administrative|clerk|receptionist|data capturer|office assistant|personal assistant|secretary|service administrator|payroll administrator|hr clerk|finance assistant)\b/.test(title)) {
      score += 120;
    }

    if (/\b(admin|administrative|office|clerical|hr|finance|reception|support services)\b/.test(category)) {
      score += 35;
    }

    if (/\badmin|administrator|administration|administrative|clerk|receptionist|data capture|data capturer|filing|office|payroll|invoices|records|documents\b/.test(combinedAll)) {
      score += 35;
    }

    if (/\bmanager|foreman|technician|electrician|chef|guide|salesperson|financial adviser|field operations manager\b/.test(title) && !/\badmin|administrator|administrative|clerk|receptionist|assistant|co-ordinator|coordinator\b/.test(title)) {
      score -= 65;
    }
  }
...
function dedupeJobs(jobs = []) {
  ...
  for (const job of jobs) {
    const source = normalizeLower(job.external_source || job.sourceType || job.source_type || 'unknown');
    ...
    const url = normalizeLower(job.applyUrl || job.apply_url || job.sourceUrl || job.source_url);

    const key = externalId ? `${source}:${externalId}` : `${title}:${company}:${area}:${url}`;
...
function sortJobs(jobs = [], rawQuery = '', requestedArea = '') {
...
async function fetchJson(url, options = {}) {
  ...
  const timeout = setTimeout(() => controller.abort(), Number(process.env.JOB_FETCH_TIMEOUT_MS || 15000));
  ...
  try {
    ...
    return {
      ok: false,
      status: 0,
      error: error?.name === 'AbortError' ? 'Request timed out' : error?.message || 'Fetch failed',
      data: null,
    };
  } finally {
...
function normalizeAdzunaJob(job = {}, meta = {}) {
  const location = normalizeText(job?.location?.display_name || job?.location || meta.location || 'South Africa');
  ...
  return {
    ...
    category: normalizeText(job?.category?.label || job?.category?.tag || job?.category || 'Jobs'),
...
function normalizeJoobleJob(job = {}, meta = {}) {
  ...
  const company = extractCompanyName(job.company || job.companyName || job.source, 'Company not stated');
  const createdAt = safeDate(job.updated || job.updated_at || job.date) || new Date().toISOString();

  return {
    id: `jooble-${normalizeText(job.id || applyUrl || `${job.title}-${company}-${location}`).replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 80)}`,
...
async function fetchAdzunaJobs(keyword, location, page = 1, resultsPerPage = 50) {
  if (!ADZUNA_APP_ID || !ADZUNA_APP_KEY) {
    return { ok: false, jobs: [], count: 0, total: 0, skipped: true, error: 'Adzuna is not configured' };
  }
  ...
  if (!response.ok) {
    return { ok: false, jobs: [], count: 0, total: 0, skipped: false, error: response.error };
  }
...
async function fetchJoobleJobs(keyword, location, page = 1, resultsPerPage = 50) {
  if (!JOOBLE_ENABLED) {
    return { ok: true, jobs: [], count: 0, rawCount: 0, total: 0, error: 'Jooble disabled' };
  }

  if (!JOOBLE_API_KEY) {
    return { ok: false, jobs: [], count: 0, rawCount: 0, total: 0, error: 'Jooble is not configured' };
  }
  ...
  if (!response.ok) {
    return { ok: false, jobs: [], count: 0, rawCount: 0, total: 0, error: response.error };
  }
...
function dbRowFromJob(job = {}, authorId = null) {
  ...
  return {
    ...
    external_id: normalizeText(job.external_id || job.id || `${job.title}-${job.company}-${job.area}`),
    title: normalizeText(job.title || 'Untitled job'),
    company: extractCompanyName(job.company),
    ...
    last_seen_at: now,
    updated_at: now,
    created_at: safeDate(job.created_at || job.createdAt) || now,
  };
}
...
async function fetchSavedJobs(rawQuery = '', area = '', limit = 50) {
  if (!supabase) {
    return { ok: false, jobs: [], count: 0, error: 'Supabase is not configured' };
  }
  ...
  try {
    ...
    if (error) {
      return { ok: false, jobs: [], count: 0, error: error.message };
    }
    ...
    if (cleanArea) {
      ...
      jobs = jobs.filter((job) => {
        const combined = normalizeLower(`${job.area} ${job.town} ${job.province} ${job.description}`);
        return combined.includes(areaLower) || combined.includes('limpopo') || combined.includes('south africa');
      });
    }

    return { ok: true, jobs, count: jobs.length, error: null };
  } catch (error) {
    return { ok: false, jobs: [], count: 0, error: error?.message || 'Could not fetch saved jobs' };
  }
}

async function saveJobsToSupabase(jobs = []) {
  if (!supabase) {
    return { savedToSupabase: 0, saveError: 'Supabase is not configured' };
  }

  if (!Array.isArray(jobs) || jobs.length === 0) {
    return { savedToSupabase: 0, saveError: null };
  }
  ...
  if (!authorId) {
    return {
      savedToSupabase: 0,
      saveError: 'No author_id found. Add JOB_SYSTEM_AUTHOR_ID in Render using your profiles.id value.',
    };
  }

  const rows = dedupeJobs(jobs)
    .filter((job) => !job.isSourceCard)
    .filter((job) => normalizeText(job.apply_url || job.applyUrl || job.source_url || job.sourceUrl))
  ...
  try {
    ...
    if (error) {
      return { savedToSupabase: 0, saveError: error.message };
    }

    return { savedToSupabase: data?.length || rows.length, saveError: null };
  } catch (error) {
    return { savedToSupabase: 0, saveError: error?.message || 'Could not save jobs' };
  }
}
...
function buildOfficialCards(rawQuery = '', area = '') {
  ...
  return OFFICIAL_SOURCES.filter((source) => {
    const combined = normalizeLower(`${source.title} ${source.company} ${source.area} ${source.category}`);
    ...
    id: `official-${index}-${source.company.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}`,
...
async function runCombinedSearch({
  ...
  if (includeExternal) {
    for (const location of searchPath) {
      for (const keyword of keywordPath.slice(0, 4)) {
        ...
        const adzunaResult = await fetchAdzunaJobs(keyword, location, 1, 50);
    ...
    for (const location of searchPath) {
      for (const keyword of keywordPath.slice(0, 4)) {
        ...
        const joobleResult = await fetchJoobleJobs(keyword, location, 1, 50);
        ...
        if (externalJobs.length === beforeCount && joobleResult.rawCount > 0 && joobleResult.count === 0) {
  ...
  merged = sortJobs(merged, rawQuery, area).slice(0, limit);

  const jobsToSave = merged.filter((job) => ['adzuna', 'jooble'].includes(job.external_source));
  ...
  return {
    ...
    providerStatus: {
      ...
      adzuna: {
        ...
        count: adzunaAdded,
        error: attempts.adzuna.find((item) => item.error && !item.skipped)?.error || null,
      },
      jooble: {
...
router.get('/adzuna-test', async (req, res) => {
  ...
  const result = await fetchAdzunaJobs(keyword, area, 1, Number(req.query.limit || 10));
...
router.get('/jooble-test', async (req, res) => {
  ...
  const result = await fetchJoobleJobs(keyword, area, 1, Number(req.query.limit || 10));
...
router.get('/auto-search', async (req, res) => {
  ...
  const noCache = String(req.query.nocache || req.query.noCache || 'false').toLowerCase() === 'true';
  ...
  if (!noCache && cached && Date.now() - cached.createdAt < CACHE_TTL_MS) {
    return res.json({
      ...cached.data,
      cached: true,
      cacheMinutes: CACHE_TTL_MS / 60000,
...
router.post('/', async (req, res) => {
  ...
  const authorId = normalizeText(body.author_id || body.authorId) || await resolveSystemAuthorId();
  ...
  const job = {
    ...
    verification_status: body.verification_status || body.verificationStatus || 'needs_verification',
